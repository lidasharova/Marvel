...
function getCharacterCards(characters) {
  ...
  for (let i = 0; i < characters.length; i++) {
    ...
    characterCards.push(getCharacterCards(character));
  }
...
//более короткий вариант с методом .map : 
// return characters.map(el => getCharacterCards(el));
...
function getCharacterModals(characters) {
  ...
  // //более короткая запись с методом .map
